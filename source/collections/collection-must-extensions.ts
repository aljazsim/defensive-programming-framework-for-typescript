 ...
 */
export function mustBeEmpty<T>(value: Array<T>): Array<T>
{
	if (!isEmpty(value))
	{
 ...
 */
export function mustBeNullOrEmpty<T>(value: Array<T>): Array<T>
{
 ...
 * @template T - The value type.
 * @param {Array<T>} value - The value.
 * @param {(T) => boolean} func - The selecor function.
...
export function mustContain<T>(value: Array<T>, func: (T) => boolean): Array<T>
{
	...
	{
		throw new ArgumentError("Value must contain specified expression.");
	}
